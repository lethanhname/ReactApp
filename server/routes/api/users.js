        ...
        if (!_.isEmpty(usr)) {
            ...
            bcrypt.hash(req.body.password, saltRounds, (err, hash) => {
                ...
                newUser.save()
                    ...
                    .catch((saveerr) => res.json(saveerr));
            });
        }
    })
...
router.post('/login', (req, res) => {
    User.find({ username: req.body.username })
        .then((usr) => {
            if (!_.isEmpty(usr)) {
                bcrypt.compare(req.body.password, usr[0].password, (err, ress) => {
                    if (ress === true) {
    ...
    User.find({ 'social_signin.id': req.body.social_signin.id })
        .then((user) => {
            ...
            if (_.isEmpty(user)) {
                newUser.save()
                    .then((newuser) => {
                        data = {
                            ...
                            social_signin: newuser[0].social_signin,
                            name: newuser[0].name
                        };
                        res.json({ code: 201, message: `user created using ${req.body.social_signin.name} account`, data });
                ...
                res.json({ code: 208, message: `user already exists while signing in with ${req.body.social_signin.name} account`, data });
